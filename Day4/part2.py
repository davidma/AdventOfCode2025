...
while True:
    ...
    rolls_removed_this_time = False

    ## Iterate through list of roll locations
    for(i,j) in rolls:
        ...
        if is_reachable_roll(i, j, values, max_neighbor_rolls):
            ...
            ## Update the totals
            rolls_removed_this_time = True
            result += 1
    
    ## If we cant remove anything any more, give up
    if not rolls_removed_this_time:
        break
